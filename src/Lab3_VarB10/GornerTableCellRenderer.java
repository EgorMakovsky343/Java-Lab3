...
// стога сена - таблица
    ...
    private Color specialColor = Color.GRAY; // Or any color you prefer
...
// окрасить задний фон панели в красный цвет
        ...
        } else if (ChangeVisualisator(formattedDouble)) {
            panel.setBackground(specialColor);
        } else {
// Иначе - в обычный белый
    ...
    private boolean ChangeVisualisator(String formattedDouble) {
        int dotIndex = formattedDouble.indexOf('.');
        if (dotIndex == -1 || dotIndex == formattedDouble.length() - 1) {
            return false;
        }

        String fractionalPart = formattedDouble.substring(dotIndex + 1);
        int sum = 0;
